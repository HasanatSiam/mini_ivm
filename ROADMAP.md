...
- [x] **Implement `AVG`:** Support `AVG` by tracking both `SUM` and `COUNT` internally.
- [x] **Single-Table `MIN`/`MAX`:** Maintain extrema efficiently when rows are deleted or updated.
- [x] **Multi-Table Joins:** Extend AST parser to understand `JOIN` queries. Create triggers on *all* referenced base tables and evaluate delta join rules against current base table states.
